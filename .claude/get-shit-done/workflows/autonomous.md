...
```bash
INIT=$(node "/teamspace/studios/this_studio/dikders/.claude/get-shit-done/bin/gsd-tools.cjs" init milestone-op)
```
...
```bash
DETAIL=$(node "/teamspace/studios/this_studio/dikders/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node "/teamspace/studios/this_studio/dikders/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "/teamspace/studios/this_studio/dikders/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "/teamspace/studios/this_studio/dikders/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node "/teamspace/studios/this_studio/dikders/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
